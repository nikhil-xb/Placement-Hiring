    ...
    //     // move the uploaded (temporary) file to the specified destination
        ...
        // {
            ...
            if($row)
            {
                ...
                cgpa =? ,mark12=?,mark10=?,department=?,gradyear=? where StudentId=?
                    ");
                $stmt->bind_param("ssssssss",$gpa,$mark12,$mark10,$dept,$gradyear,$sid);
                $rest= $stmt->execute();
                if(!$rest){
                    echo("Error: ".$stmt->error);}
                else
            ...
            else
            {
                ...
                (studentid,cgpa,mark12,mark10,department,gradyear,filename,file) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssssss", $sid,$gpa,$mark12,$mark10,$dept,$gradyear);
                $res= $stmt->execute();
                if(!$res)
                    echo("Error: ".$stmt->error);
                else
